% THIS PROGRAM CALCULATES SEEDED FEL AMPLIFIER WITH MICROBUNCHING-INDUCED
% SIDEBAND POWER. THE REFERENCE FORMULAS ARE REFERRED TO Z.ZHANG ET AL.
% PRAB 19, 050701 (2016)
...
if (length(PS_phi_array)>5)
    ...
    figure(102); subplot(2,1,1); plot(und_L_array,abs(Power_und(1:end-1))/1e3,'rs'); xlabel('Radiator s (m)'); ylabel('P_{out} (GW)'); hold on;
    figure(102); subplot(2,1,2); plot(und_L_array,abs(b_und(1:end-1)),'bs'); xlabel('Radiator s (m)'); ylabel('|b|'); hold on;
    figure(102); subplot(2,1,2); plot([0 und_L_array(end)],[1 1],'k--'); hold on;
end
%}
...
fprintf('========= EXTERNAL SEED FEL PERFORMANCE ESTIMATE =========\n');
if (MX==0)
