...
% format (s,R11,R12,...,R65,R66)
filename1='lattice_transport_functions.o';
delimiterIn=' '; headerlinesIn=0;
transport=importdata(filename1,delimiterIn,headerlinesIn);
s_ele=transport(:,1);
transport_ELEGANT=transport;

% minus-sign issue due to ELEGANT defnition z<0 for bunch head
...
% now, z>0 for bunch head

% base conversion from (x,px,y,py,z,delta) to (x,px,y,py,z,Delta_gamma)
transport(:,7)=transport(:,7)/eegamma;
transport(:,13)=transport(:,13)/eegamma;
transport(:,19)=transport(:,19)/eegamma;
transport(:,25)=transport(:,25)/eegamma;
transport(:,31)=transport(:,31)/eegamma;
transport(:,32)=transport(:,32)*eegamma;
transport(:,33)=transport(:,33)*eegamma;
transport(:,34)=transport(:,34)*eegamma;
transport(:,35)=transport(:,35)*eegamma;
transport(:,36)=transport(:,36)*eegamma;
%-------------------------------------------------------------------------%
end
