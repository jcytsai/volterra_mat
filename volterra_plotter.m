...
% Last Modified by GUIDE v2.5 31-Aug-2016 09:41:32
...
if ((scan_num==1))
    ...
    %}
    if (min(isnan(G_11))~=1)
        set(gca,'FontSize',40,'linewidth',5); plot(s/100,G_11,'k-','linewidth',3);         xlabel('s (m)'); ylabel('G(s)');  grid off; axis('tight');
    else
        msgbox('no initial density modulation exists...','','warn');
    end
    %end
else
...
if (scan_num>=2)
        ...
        %}
        if (min(isnan(Gf_11))~=1)
            set(gca,'FontSize',40,'linewidth',5); plot(lambda_array*10^4,Gf_11,'k-','linewidth',3); xlabel('\lambda (\mum)'); ylabel('G_{f}');  grid off; axis('tight');
        else
            msgbox('no initial density modulation exists...','','warn');
        end
        %end
...
if (scan_num>=2)
    if (iEnergy_gain_calc==0)
        figure(305); 
        if (min(isnan(G_total))==0)
            set(gca,'FontSize',40,'linewidth',5); surf(lambda_array*10^4,s/100,abs(G_total)); view(-38,32); xlabel('\lambda (\mum)','FontSize',40); ylabel('s (m)','FontSize',40); zlabel('G(s,\lambda)','FontSize',40); axis('tight'); 
        %else
        %    set(gca,'FontSize',40,'linewidth',5); surf(lambda_array*10^4,s/100,abs(G_c_11)); view(-38,32); xlabel('\lambda (\mum)','FontSize',40); ylabel('s (m)','FontSize',40); zlabel('G(s,\lambda)','FontSize',40); axis('tight'); 
        %end
    ...
    if (iEnergy_gain_calc==1)
        figure(306); 
        if (min(isnan(E_total))==0)
            set(gca,'FontSize',40,'linewidth',5); surf(lambda_array*10^4,s/100,abs(E_total)); view(-38,32); xlabel('\lambda (\mum)','FontSize',40); ylabel('s (m)','FontSize',40); zlabel('E(s,\lambda)','FontSize',40); axis('tight'); 
        %else
        %    set(gca,'FontSize',40,'linewidth',5); surf(lambda_array*10^4,s/100,abs(G_c_44)); view(-38,32); xlabel('\lambda (\mum)','FontSize',40); ylabel('s (m)','FontSize',40); zlabel('E(s,\lambda)','FontSize',40); axis('tight'); 
        %end
        shading('interp'); rotate3d on; h=colorbar(); set(h,'FontSize',30);
        end
    end
...
load('workplace.mat');
if (scan_num>=2)
    if (min(isnan(Gf_44))~=1)
        set(gca,'FontSize',40,'linewidth',5); plot(lambda_array*10^4,Gf_44,'k-','linewidth',3); xlabel('\lambda (\mum)'); ylabel('|p_k^{e}|, energy-to-energy');  grid off; axis('tight');
    else
        msgbox('no initial energy modulation exists...','','warn');
    end
else
...
if ((scan_num==1))
    if (min(isnan(G_44))~=1)
        set(gca,'FontSize',40,'linewidth',5); plot(s/100,G_44,'k-','linewidth',3);    xlabel('s (m)'); ylabel('G_{44}(s), energy-to-energy');  grid off; axis('tight'); hold off;
    else
        msgbox('no initial energy modulation exists...','','warn');
    end
else
...
%global mag_layout;
load('workplace.mat'); load('mag_layout.o');
scale_factor=0.1*(abs(max(G_44)-min(G_44)));
if ((scan_num==1))
    if (min(isnan(G_44))~=1)
        set(gca,'FontSize',40,'linewidth',5); plot(s/100,G_44,'k-','linewidth',3);    xlabel('s (m)'); ylabel('G_{44}(s), energy-to-energy'); hold on; grid off; axis('tight');
...
%global mag_layout;
load('workplace.mat'); load('mag_layout.o');
scale_factor=0.1*(abs(max(G_11)-min(G_11)));
if ((scan_num==1))
    %{
    if (iiterative==1 && ilast==0)
        set(gca,'FontSize',40,'linewidth',5); plot(s/100,G_zero,'r-','linewidth',3);    xlabel('s (m)'); ylabel('G(s)'); hold on; grid off; axis('tight');
    ...
    %}
    if (min(isnan(G_11))~=1)
        set(gca,'FontSize',40,'linewidth',5); plot(s/100,G_11,'k-','linewidth',3);         xlabel('s (m)'); ylabel('G(s)'); hold on; grid off; axis('tight');
        set(gca,'FontSize',40,'linewidth',5); area(mag_layout(:,1),scale_factor*mag_layout(:,2),'FaceColor', 'r', 'EdgeColor', 'r'); hold off; axis('tight');
    else
        msgbox('no initial density modulation exists...','','warn');
    end
