...
if (iIBS==1)
    ...
    if (iSES_ana==1)
        fprintf('final pure optics SES %.4e keV, total SES %.4e keV (analytical %.4e keV, %.4e keV), extra IBS SES %.4e keV...\n',sqrt(tmp05)*1e3,func*1e3,func_ana*1e3,func_ana_ana*1e3,sigma_delta*egamma*mc2*1e3);
    else
        %fprintf('SES contributed by IBS (%.1f%%) and by collective effect (%.1f%%)...\n',sqrt(tmp06)/func*100,sqrt(tmp04)/func*100);
        fprintf('final pure optics SES %.4e keV, total SES %.4e keV, extra IBS SES %.4e keV...\n',sqrt(tmp05)*1e3,func*1e3,(sigma_delta_IBS(end)-sigma_delta)*egamma*mc2*1e3);
    end
else
    ...
    if (iSES_ana==1)
        fprintf('final pure optics SES %.4e keV, total SES %.4e keV (analytical %.4e keV, %.4e keV)...\n',sqrt(tmp05)*1e3,func*1e3,func_ana*1e3,func_ana_ana*1e3);
    else
        %fprintf('SES contributed by collective effect (%.1f%%)...\n',sqrt(tmp04)/func*100);
        fprintf('final pure optics SES %.4e keV, total SES %.4e keV...\n',sqrt(tmp05)*1e3,func*1e3);
    end
end
