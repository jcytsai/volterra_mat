function func=kernel_mod_M(a,b)    
    ...
    tmp01=k_wave^2*re*nb./egamma;          % corrected on 20200628
    ...
    %if (iIBS==0)
        func=tmp01.*(tmp02.^2).*tmp03*sigma_delta^2.*tmp04.*tmp05.*exp(-tmp06.*tmp07-tmp09.*tmp10-tmp08);
    %else
    %    func=tmp01.*(tmp02.^2).*tmp03.*sigma_delta_IBS(1:).^2.*tmp04.*tmp05.*tmp12.*exp(-tmp06.*tmp07-tmp09.*tmp10-tmp08);
    %end
