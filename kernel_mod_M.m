function func=kernel_mod_M(a,b)    
    ...
    %tmp01=1i*k_wave^2*re*nb./egamma;          % constant term
    tmp01=k_wave^2*re*nb./egamma;             % constant term, corrected on 20200615
    ...
    tmp12=R56transport(b,a);                                                   % R56(s'->s), vector
    
    %if (iIBS==0)
        func=tmp01.*(tmp02.^2).*tmp03*sigma_delta^2.*tmp04.*tmp05.*tmp12.*exp(-tmp06.*tmp07-tmp09.*tmp10-tmp08);
    %else
    %    func=tmp01.*(tmp02.^2).*tmp03.*sigma_delta_IBS(1:end-1).^2.*tmp04.*tmp05.*tmp12.*exp(-tmp06.*tmp07-tmp09.*tmp10-tmp08);
    %end
    
    if (itransLD==0)
        func=tmp01.*(tmp02.^2).*tmp03*sigma_delta^2.*tmp04.*tmp05.*tmp12.*exp(-tmp08);
    end
