...
if (iIBS==1)
    iEnergy_gain_calc=0;                 % optional
    iTransverse_gain_calc=0;             % optional
...
%----------------------Solve Volterra integral equation-------------------%
    ...
    if (iIBS==1); s_IBS=s; IBS_calc; end
    ...
    if (iEnergy_gain_calc==0)
        if (iiterative==0)
            ...
            if (iIBS==1)
                ...
                E0_k=p0k_mat(s);
                IBS_calc;
                if (HOM_IBS==1 && ((iCSR_drift==1) || (iLSC==1) || (ilinac==1)))
                    tmp02=1i*k_wave*diag(int_IBS_growth_rate_02)*inv(eye(mesh_num)+2*diag(int_IBS_growth_rate_01));
                    one_minus_K_mat=eye(mesh_num)-mesh_size*K_mat-diag(int_IBS_growth_rate_01)-tmp02*M_minus_L_mat;
                else
                    tmp02=zeros(mesh_num,mesh_num);
                    one_minus_K_mat=eye(mesh_num)-mesh_size*K_mat-diag(int_IBS_growth_rate_01);  % use minus sign
                    %one_minus_K_mat=eye(mesh_num)-mesh_size*K_mat+diag(int_IBS_growth_rate_01);  % use plus sign
                end
                tmp03=tmp02*transpose(E0_k);
                gkd_mat=one_minus_K_mat\(transpose(G0_k)+tmp03);
            end
            %}
        else
