function func=p0kxpz_mat(s)
    ...
    tmp12=(betax0*gammax0+alphax0^2)*tmp03;
    tmp13=2*alphax0*betax0*tmp02;
    
    LD=exp(-(tmp07).*(tmp08)-(tmp09).*(tmp10)-tmp11);
    
    func=(k_wave*tmp01).^2.*tmp06*sigma_delta^2*axp_1k0.*(tmp12-tmp13).*LD;
