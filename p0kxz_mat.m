function func=p0kxz_mat(s)
    ...
    tmp12=2*alphax0*gammax0*tmp03;
    tmp13=(betax0*gammax0+alphax0^2)*tmp02;
    
    LD=exp(-(tmp07).*(tmp08)-(tmp09).*(tmp10)-tmp11);
    
    func=(k_wave*tmp01).^2.*tmp06*sigma_delta^2*ax_1k0.*(tmp12-tmp13).*LD;
