function func=p0kypz_mat(s)
    ...
    tmp12=(betay0*gammay0+alphay0^2)*tmp05;
    tmp13=2*alphay0*betay0*tmp04;
    
    LD=exp(-(tmp07).*(tmp08)-(tmp09).*(tmp10)-tmp11);
    
    func=(k_wave*tmp01).^2.*tmp06*sigma_delta^2*ayp_1k0.*(tmp12-tmp13).*LD;
